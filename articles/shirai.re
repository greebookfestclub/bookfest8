...
NTTドコモ副島義貴さんと、ラボの発信活動でご協力いただいている産業用VRのスタートアップ企業・イクスアール株式会社COO小池健輔さん、そしてVR Studio Labのインターンであり、東京工業大学発スタートアップ企業「Hapbeat合同会社」の代表でもある山崎勇祐さんと共著する形で企画提案をエントリーし、みごと採択されました（@<href>{https://sa2019.siggraph.org/attend/real-time-live/session/111}）@<bib>{VRON20191119}。
...
（5）ダンスや歌はしっかり楽しい
という構成です。
システム的にはコードネーム REMOPRE、後に「Viveshare」と名付けた
次世代ライブエンタテイメントのための技術開発が始まりました。


=== チーム編成と技術構成

まずは今回のデモを成立させる上で、また「直感アルゴリズム」の出演と双方向触覚を実装する上で、``全部盛り''の状態から
戦略的な取捨選択を行いました。
昨年のようにWFLE PTグループで開発しているUnreal Engine 4（UE4）ベースで開発されているスタジオのエンジンを続けて
RTLのデモに使用することも魅力的なのですが、
...
昨年は日本語と英語のバイリンガルですすめるコストがありましたが、今回はシナリオと実演以外は日本語だけで進行できそうです（これが後で別の音声系の 問題を生んだりもしました）。
