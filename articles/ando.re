...
== 前提条件
本資料を読んでいただく為の前提条件を示します。

・JavaやC#などのプログラムを組んだことがあるが、C++言語のプログラムをあまり知らない方（必須）
・C++言語をこれから勉強したい人 (本資料を注意しながら勉強してほしい)
...
=== new / delete
C++言語を触る上で
　new と delete (もしくはC言語のmalloc / free)
の意味がわからない方はC++言語を勉強し直し、必ず理解してください。
new/deleteはメモリ確保/解放処理そのものです。
...
* 継承される可能性があるクラスのデストラクタは必ずvirtual宣言をする。virtual ~Hoge();
** virtualを記述しないとで基底クラス（スーパークラス）のデストラクタを呼んでも、継承先のクラス（サブクラス）のデストラクタがコールされず、リークが発生する恐れがある。
** 仮想関数を持つクラスと持たないクラスのサイズ差がたかだか4(or 8)バイトなので、基本的に付ける方針で問題ないと思う。
* 中身が空のデストラクタを実装するのがイヤなら、ヘッダーに次のように記述する。virtual ~Hoge()=default;
** コンパイラが自動的に処理してくれるので、ビルド速度もあがって良い感じ。（C++11から）
* デストラクタ内で記述してよいのは下記のとおり。仕様上はなんでも記述できるが、デバッグが死ぬほど大変になるので、ルールとして心がけておく
** メンバ変数（ポインタ）のdelete
** リソース（テクスチャはモデルなど）の解放処理
** クリティカルセクションのunlock
** スレッドや非同期処理の停止
...
ポインタを理解した上で、以降を読む際に押さえて欲しいことは以下の内容です。
* ポインタ変数は4バイト ※サイズはsizeof(int*)等で調べられます。(64bit環境では8バイト）

== 基礎知識
=== 参照(&)について
参照は身に付けておくべき項目です。
C++以降の言語の変数は実際は参照となっています。（個人的見解なので、間違っていたらごめんなさい）
...
//}
上記した通り、ポインタ変数のサイズは4バイトで、アドレスの先の実体は変わりません。
...
メンバの場合はコンストラクタで、ローカル変数の場合は生成時に必ず初期値を設定しましょう。
...
そこで生み出された仕組みがGCです。このことは頭の片隅にでも記憶していてください。
...
これでポインタ管理、メモリ管理(delete)を行わなくて済みます。
...
C++11はC#を触ったことのある方にとっては、馴染みやすいと思います。
※ C++11は通称。2011年にISO/IEC 14882:2011として改定されたC++のバージョンのことを言います。
　なお、C++11以降でC++14(2014年にISO/IEC 14882:2014として改訂)がありますが、C++11ほどのインパクトはありません。
...
従って、よくわからないマクロが出てきた場合は、遠慮せず有識者に教わってください。
