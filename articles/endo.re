...
== WFSとは
WFS@<fn>{endofn01}とはグリーの子会社で、スマートフォン向けのゲームアプリを制作・運営しています。WFSでは設立当初から、既存のグリープラットフォームから独立した、いわゆる普通のスマートフォン向けゲームアプリを作ろうとして設立されました。既存のグリープラットフォームというのは、いわゆるガラケー時代から続いているものです。これはグリーのアカウントを作ることがほぼ前提となっているため、今のスマホアプリに対しては合わない部分もありました。しかしこちらを使わないこととなると、既に存在していた便利なものを全て自前で作ることになります。そこで新たな共通の基盤が必要となり、開発がスタートしました。
