...
@<br>{}
このOOPはプログラミング技術の差が非常に出やすく、初級プログラマがクラス設計を行うと「クラスを使わなかった方がマシだった」とレビューされる事も珍しくありません。
@<br>{}
...
本稿では初心者でも分かるようにSOLIDの概念を要約する形で解説しているため、厳密な説明は行っていませんが、なんとなく概要だけでも覚えてください。
...
==={リスコフの置換原則-目的} 目的
 * ダウンキャストを使わない設計を目指す
//list[downcast][downcast][c++]{
ExXxxClass xxx = (ExXxxClass)obj;
}
==={リスコフの置換原則-概要} 概要
ダウンキャストをしなければいけない設計は、大抵例外的な処理を書くことになります。
XxxClassを継承したXxx2Classという新しいクラスを作ったとき
ExXxxClassがダウンキャストした箇所はXxx2Classもダウンキャストして独自の処理を呼ばなければバグる、
なんて設計になっていたことがよくあります。
これの問題は@<b>{知らなければバグる部分}が存在してしまうことです。
プロジェクトはどんどん人が入れ替わります。
誰かに教えてもらわないとミスに繋がりそうな設計をしないことが業務プログラマとして必要です。
...
依存性注入とはクラスの中で必要になる処理をクラスの外で記述し渡してもらう方法です。std::sortのようにソートする基礎ロジックは内部に持ってるけど、ソート条件は外からもらう、みたいな動きのことです。
===[/column]
