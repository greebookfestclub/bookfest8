...
最近の趣味はMicroServicesと分散システムとなりますので、JWTの認証サーバを作ったり、gRPCを触ったりしています。
本ドキュメントは、私が担当したモニタリングの仕事、外形監視の導入をベースで、いくつのインフラ関連の自動化ツールと使い方をご紹介します。
...
=== サービス管理者視点
インフラの運用者はサーバのCPU負荷、メモリーの使用量を監視したい、サービスの開発者はアプリのエラーログを監視して修正を行いたいとか、
そういった要件を満たす監視は内部監視・もしくはホワイトボックス監視とよく呼ばれております。
...
== 外製の外形監視サービス
グリではもともと内製の外形監視サービスを使ってましたが、メンテの手間や検知誤差の原因で、外製のサービスに移行しました。
外形監視業界では、代表的なサービスはPingdomがあります。
Pingdomはたくさんの機能を搭載しているプラットホームです。監視以外アラート通知やサイトパーフォマンス分析など豊かな機能もあります。
AWSやGCP（stackdriver）などパブリッククラウドにも外形監視のサービスがあります。
いろんな外形監視サービスがあると思いますが、基本的にhttp/https/ping監視のベース機能が入っていますので、
自分の要件を合わせて適切なサービスを選択するのはいいと思います。

=== UptimeRobot
今回特に紹介したいサービスはUptimeRobotというサービスになります。
...
さらに、UptimeRobotは無料で50モニタを設定することができるので、検証したい・すぐ使いたい方にはすごい便利なサービスだと思います。
監視の設定に関しては詳しく説明をしないが、公式サイト@<fn>{lifn02}で実際に手を動かしてみた方が早いと思います。
//footnote[lifn02][https://uptimerobot.com/]

== インフラでの外形監視構築と運用
上までモニタリングや外形監視についてお話しました。
実際にUptimeRobotを利用して実際にサンプルを紹介する前に、まず１つの概念をご紹介したいと思います。

=== IaC
Infrastructure as Code.
文字のどおり、インフラの設定をコードとして書いて関することです。
GUIに通らず、コードを書く・更新でインフラの構築や修正ができるメリットは、
1. インフラ設定の記録はコードの記録として残されますので、監査しやすい。
2. インフラ環境の複製が簡単にできます。
3. コードがあれば、CI/CDツールを使って自動化することができます。
となります。

また、APIでインフラプラットホームと通信できることは、IaCツールが使える前提となります。
インフラプラットホームといえば、先にAWSやGCPと思う方が多いかもしれませんが、
上紹介したUptimeRobotもある意味ではインフラプラットホームです。

この後に、IaCツールを使ってUptimeRobotに監視モニターを設定するサンプルを紹介します。
 ...
 * tf.state
 ** Terraformとインフラをシンクするための状態ファイル、なくなったら大惨事になります。
 ...
 * 新規ジョブ作成 : フリースタイル・プロジェクトのビルドを選択する
 ...
 * ジョブ設定 -> ビルド・トリガ
  ** GitHub hook trigger for GITScm polling」をチェックする
 * ジョブ設定 -> ビルド -> シェルの実行
...
最後に外形監視（UptimeRobot）を運用するDemoを紹介しましたが、いくつの注意点があります。
 ...
 2. UptimeRobotのapi_keyは直接にスクリプトに書いてますが、
 実際の場合、Jenkinsのsecret textなどで環境変数を渡す方が安全だと思います。
