...
（しかし、レビュー段階で指摘しにくいのでできるだけホスピタリティを意識してください）
適切なネスト深さ心がける
４段、５段になる場合、ネストの中身を別のメソッドに置き換えたほうが見やすい。
またラムダ式を入れ子にして深いネストになるようなことを可能な限り避ける。
一行に長過ぎる処理を記述しない。
次のデバッグしやすさにも起因するのですが、長い数式や複数の処理を一行にまとめてしまうと、レビュアーが見づらいだけではなくデバッガでステップ実行の際に変数の変更を追うのが難しくなります。
基本的には下記のように心がけてください
1行には1命令
テキストエディタを横スクロールしないと見れないような長い式は分割する。
if文などで、括弧の中にメソッドを記述しない。（変数等に戻り値を保持して、それをconditionとする）
&&演算子、||演算子を使いすぎない。
...
同一のロジックが2箇所以上で書かれていないか
ただし、ラムダ式などでごく短いセンテンスが重複する程度（５行程度）は、許容する（インライン展開での実行速度を考慮して）
