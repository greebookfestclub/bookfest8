...
//image[SA19-scenario,SA19RTLeq][（左）シナリオと技術的な実装要素／（右）筆者・メインMCのハードウェア装備]{
//}
...
Main,OMENそれぞれのPCで @<code>{node index.js} を起動し（おススメは VS Code のコンソールで投入します）、ブラウザでURLを入力することで、各PC上で必要となる可視化や通信が行われる仕組みになっています。
